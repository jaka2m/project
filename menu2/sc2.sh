...
# Fungsi untuk menginstal dependensi khusus
function install_dependencies(){
# Mendeteksi OS dan versi
OS=$(grep -E '^ID=' /etc/os-release | cut -d'=' -f2)
VERSION_ID=$(grep -E '^VERSION_ID=' /etc/os-release | cut -d'=' -f2 | tr -d '"')

    echo "Mendeteksi OS: $OS $VERSION_ID"
    if [[ "$OS" == "ubuntu" ]]; then
        if [[ "$VERSION_ID" == "22.04" || "$VERSION_ID" == "24.04" ]]; then
            echo "Menginstal dependensi untuk Ubuntu 22.04/24.04..."
            wget http://archive.ubuntu.com/ubuntu/pool/main/o/openssl/libssl1.1_1.1.1f-1ubuntu2_amd64.deb
            sudo dpkg -i libssl1.1_1.1.1f-1ubuntu2_amd64.deb
            sudo apt --fix-broken install -y
            sudo apt install -y liblua5.3-0
        else
            echo "Versi Ubuntu ini tidak memerlukan instalasi dependensi libssl1.1."
        fi
    elif [[ "$OS" == "debian" ]]; then
        if [[ "$VERSION_ID" == "11" || "$VERSION_ID" == "12" ]]; then
            echo "Menginstal dependensi untuk Debian 11/12..."
            ...
            echo "Versi Debian ini tidak memerlukan instalasi dependensi libssl1.1."
        fi
    else
        echo "OS tidak didukung atau tidak terdeteksi."
    fi
}
