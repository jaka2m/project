...
# Mendeteksi OS
OS_NAME=$(grep -w ID_LIKE /etc/os-release | cut -d= -f2 | tr -d '"')
if [ "$OS_NAME" == "debian" ]; then
  OS_TYPE="debian"
elif [ "$OS_NAME" == "ubuntu" ]; then
  OS_TYPE="ubuntu"
else
  echo -e "${RED}[ERROR]${NC} Sistem operasi tidak didukung. Skrip ini hanya untuk Debian dan Ubuntu."
  exit 1
fi
...
function first_setup() {
    ...
    echo -e " ${BLUE}        └────────────────────────────────────┘${NC}"
    echo -e "${BOLD_YELLOW}        Deteksi Otomatis OS: ${GREEN}$(cat /etc/os-release | grep -w PRETTY_NAME | head -n1 | sed 's/PRETTY_NAME=//g' | sed 's/"//g')${NC}"
    echo ""

    if [ "$OS_TYPE" == "ubuntu" ]; then
        DISTRO_CODENAME=$(grep VERSION_CODENAME /etc/os-release | cut -d= -f2)
        
        if [ "$DISTRO_CODENAME" == "jammy" ] || [ "$DISTRO_CODENAME" == "noble" ]; then
            print_install "Menyiapkan dependensi untuk Ubuntu ${DISTRO_CODENAME}"
            wget -q http://archive.ubuntu.com/ubuntu/pool/main/o/openssl/libssl1.1_1.1.1f-1ubuntu2_amd64.deb
            sudo dpkg -i libssl1.1_1.1.1f-1ubuntu2_amd64.deb > /dev/null 2>&1
            sudo apt --fix-broken install -y > /dev/null 2>&1
            sudo apt install -y liblua5.3-0 > /dev/null 2>&1
        fi
        
        print_install "Memulai instalasi HAProxy untuk Ubuntu $(cat /etc/os-release | grep -w PRETTY_NAME | head -n1 | sed 's/PRETTY_NAME=//g' | sed 's/"//g')"
        ...
        if [ "$DISTRO_CODENAME" == "noble" ]; then
            ...
            apt-get -y install haproxy socat > /dev/null 2>&1
        else
        ...
        if [ "$DEBIAN_VERSION" == "11" ] || [ "$DEBIAN_VERSION" == "12" ]; then
            print_install "Menyiapkan dependensi untuk Debian ${DEBIAN_VERSION}"
            wget -q https://security.debian.org/pool/updates/main/o/openssl/libssl1.1_1.1.1w-0+deb11u3_amd64.deb
            sudo dpkg -i libssl1.1_1.1.1w-0+deb11u3_amd64.deb > /dev/null 2>&1
            sudo apt --fix-broken install -y > /dev/null 2>&1
            sudo apt install -y liblua5.3-0 > /dev/null 2>&1
        fi

        print_install "Memulai instalasi HAProxy untuk Debian $(cat /etc/os-release | grep -w PRETTY_NAME | head -n1 | sed 's/PRETTY_NAME=//g' | sed 's/"//g')"
        apt-get -y install --no-install-recommends software-properties-common > /dev/null 2>&1

        if [ "$DEBIAN_VERSION" == "10" ]; then
            wget -O - https://haproxy.debian.net/bernat.debian.org.gpg | sudo apt-key add - > /dev/null 2>&1
            echo "deb http://haproxy.debian.net buster-backports-2.4 main" | sudo tee /etc/apt/sources.list.d/haproxy.list > /dev/null
            sudo apt-get update -y > /dev/null 2>&1
            sudo apt install -y haproxy=2.4.\* > /dev/null 2>&1
        elif [ "$DEBIAN_VERSION" == "11" ]; then
            wget -O - https://haproxy.debian.net/bernat.debian.org.gpg | sudo apt-key add - > /dev/null 2>&1
            echo "deb http://haproxy.debian.net bullseye-backports-2.8 main" | sudo tee /etc/apt/sources.list.d/haproxy.list > /dev/null
            sudo apt-get update -y > /dev/null 2>&1
            sudo apt install -y haproxy=2.8.\* > /dev/null 2>&1
            ...
            sudo apt-get update -y > /dev/null 2>&1
            sudo apt install -y -t bookworm-backports haproxy > /dev/null 2>&1
        else
    ...
    print_success "Pemasangan HAProxy pada $(cat /etc/os-release | grep -w PRETTY_NAME | head -n1 | sed 's/PRETTY_NAME=//g' | sed 's/"//g') selesai."
}
...
if [[ $choose_domain == "2" ]]; then
    # Mengambil Domain Otomatis
    Random_Number=$( </dev/urandom tr -dc 1-$( curl -s ${GEO_VPN}ssh/domain.list | grep -E Jumlah | cut -d " " -f 2 | tail -n1 ) | head -c1 | tr -d '\r\n' | tr -d '\r')
    Domain_Hasil_Random=$( curl -s ${GEO_VPN}ssh/domain.list | grep -E Domain$Random_Number | cut -d " " -f 2 | tr -d '\r' | tr -d '\r\n')
    SUB_DOMAIN="$(</dev/urandom tr -dc a-x1-9 | head -c5 | tr -d '\r' | tr -d '\r\n')"
    EMAIL_CLOUDFLARE="desalekong24@gmail.com"
    ...
    # Mengupdate Record A (jika sudah ada)
    RESULT=$(curl -sLX PUT "https://api.cloudflare.com/client/v4/zones/${ZonaPadaCloudflare}/dns_records/${RECORD}" \
    -H "X-Auth-Email: ${EMAIL_CLOUDFLARE}" \
    -H "X-Auth-Key: ${API_KEY_CLOUDFLARE}" \
    -H "Content-Type: application/json" \
    ...
    # Menambahkan Record NS
    RECORD_NS=$(curl -sLX POST "https://api.cloudflare.com/client/v4/zones/${ZonaPadaCloudflare}/dns_records" \
    -H "X-Auth-Email: ${EMAIL_CLOUDFLARE}" \
    -H "X-Auth-Key: ${API_KEY_CLOUDFLARE}" \
    -H "Content-Type: application/json" \
    --data '{"type":"NS","name":"ns.'${SUB_DOMAIN}'","content":"'${SUB_DOMAIN}.${Domain_Hasil_Random}'","ttl":0,"proxied":false}' | jq -r .result.id)
    
    # Mengupdate Record NS (jika sudah ada)
    RESULT_NS=$(curl -sLX PUT "https://api.cloudflare.com/client/v4/zones/${ZonaPadaCloudflare}/dns_records/${RECORD_NS}" \
    -H "X-Auth-Email: ${EMAIL_CLOUDFLARE}" \
    -H "X-Auth-Key: ${API_KEY_CLOUDFLARE}" \
    -H "Content-Type: application/json" \
    --data '{"type":"NS","name":"ns.'${SUB_DOMAIN}'","content":"'${SUB_DOMAIN}.${Domain_Hasil_Random}'","ttl":0,"proxied":false}')
